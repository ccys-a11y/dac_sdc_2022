...
void Mem2Stream(ap_uint<LineWidth> *in, stream<ap_uint<LineWidth> > &out, const unsigned reps = 1)
{
...
template <unsigned LineWidth, unsigned NumLines>
void Stream2Mem(stream<ap_uint<LineWidth> > &in, ap_uint<LineWidth> *out, const unsigned reps = 1) {
    for (unsigned i = 0; i < reps * NumLines; i++) {
        out[i] = in.read();
    }
}
...
/**
 * 浠庝竴涓祦涓竴瀹氭暟閲忕殑鏁版嵁move鍒板彟涓�涓祦
 */
template <unsigned IN_BIT>
