...
void resize_batch_2(hls::stream<ap_uint<24> > &in, hls::stream<ap_uint<24> > &out,
                  unsigned int reps) {
	  hls::Mat<IN_IMAGE_HEIGHT, IN_IMAGE_WIDTH, HLS_8UC3> raw_img;
	#pragma HLS STREAM variable = raw_img depth = 1024 dim = 1    ////////////////写道主函数里
	  hls::Mat<RESIZE_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, HLS_8UC3> resize_img;
	#pragma HLS STREAM variable = resize_img depth = 1024 dim = 1  ////////////////写道主函数里
  for (unsigned int rep = 0; rep < reps; rep++) {
#pragma HLS dataflow
	  stream_to_mat(in, raw_img);
	  hls::Resize_opr_linear(raw_img, resize_img);
...
////////  [batch][height][weight][4][3]
void stream_to_mat_48(
    hls::stream<ap_uint<48> > &in,
	hls::Mat<IN_IMAGE_HEIGHT, IN_IMAGE_WIDTH, HLS_8UC3> &raw_img_1,
    hls::Mat<IN_IMAGE_HEIGHT, IN_IMAGE_WIDTH, HLS_8UC3> &raw_img_2) {

  for (int i = 0; i < IN_IMAGE_HEIGHT; i++) {
    for (int j = 0; j < IN_IMAGE_WIDTH; j++) {
#pragma HLS pipeline II = 1
      hls::Scalar<3, ap_uint<8> > pix_1;
      hls::Scalar<3, ap_uint<8> > pix_2;
      ap_uint<96> in_data = in.read();
      for (unsigned int p = 0; p < 3; p++) {

        pix_1.val[p] = in_data(8 * p + 7, 8 * p);
        pix_2.val[p] = in_data(24 + 8 * p + 7, 24 + 8 * p);
      }
      raw_img_1 << pix_1;
      raw_img_2 << pix_2;
    }
...
void mat_to_stream_2(
    hls::Mat<RESIZE_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, HLS_8UC3> &resize_img_1,
	hls::Mat<RESIZE_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, HLS_8UC3> &resize_img_2,
    hls::stream<ap_uint<24> > &out) {
...
void Resize_2_par(hls::Mat<IN_IMAGE_HEIGHT, IN_IMAGE_WIDTH, HLS_8UC3> &raw_img_1,
		hls::Mat<IN_IMAGE_HEIGHT, IN_IMAGE_WIDTH, HLS_8UC3> &raw_img_2,
		hls::Mat<RESIZE_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, HLS_8UC3> &resize_img_1,
			    hls::Mat<RESIZE_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, HLS_8UC3> &resize_img_2){

	hls::Resize_opr_linear(raw_img_1, resize_img_1);
	hls::Resize_opr_linear(raw_img_2, resize_img_2);
	}
void resize_batch_3(hls::stream<ap_uint<48> > &in, hls::stream<ap_uint<24> > &out,
                  unsigned int reps) {
	unsigned repsp2 =  reps/2 ;
	  hls::Mat<IN_IMAGE_HEIGHT, IN_IMAGE_WIDTH, HLS_8UC3> raw_img_1;
	  hls::Mat<IN_IMAGE_HEIGHT, IN_IMAGE_WIDTH, HLS_8UC3> raw_img_2;
	#pragma HLS STREAM variable = raw_img_1 depth = 1024 dim = 1    ////////////////写道主函数里
	#pragma HLS STREAM variable = raw_img_2 depth = 1024 dim = 1
	  hls::Mat<RESIZE_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, HLS_8UC3> resize_img_1;
	  hls::Mat<RESIZE_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, HLS_8UC3> resize_img_2;
	#pragma HLS STREAM variable = resize_img_1 depth = 1024 dim = 1  ////////////////写道主函数里
	#pragma HLS STREAM variable = resize_img_2 depth = 1024 dim = 1
  for (unsigned int rep = 0; rep < repsp2; rep++) {
#pragma HLS dataflow
	  stream_to_mat_48(in, raw_img_1, raw_img_2);
	  Resize_2_par(raw_img_1,raw_img_2, resize_img_1,resize_img_2);
	  mat_to_stream_2(resize_img_1, resize_img_2, out);
  }
}
//void resize_batch_pipeline(hls::stream<ap_uint<24> > &in, hls::stream<ap_uint<24> > &out,
//                  unsigned int reps) {
//  hls::Mat<IN_IMAGE_HEIGHT, IN_IMAGE_WIDTH, HLS_8UC3> raw_img;
//#pragma HLS STREAM variable = raw_img depth = 1024 dim = 1    ////////////////写道主函数里
//  hls::Mat<RESIZE_IMAGE_HEIGHT, RESIZE_IMAGE_WIDTH, HLS_8UC3> resize_img;
...
//void resize_stage1(
//    hls::stream<ap_uint<24> > &in,
//    hls::Mat<Batch_size, Pixel_2d, HLS_8UC3> &raw_img) {
//  for (int i = 0; i < Batch_size; i++) {
//    for (int j = 0; j < Pixel_2d; j++) {
//#pragma HLS pipeline II = 1
//      hls::Scalar<3, ap_uint<8> > pix;
//      ap_uint<24> in_data = in.read();
//      for (unsigned int p = 0; p < 3; p++) {
...
#pragma HLS STREAM variable = in_stream2 depth = 16 dim = 1
  resize_batch_3(in_stream1, in_stream2, reps);
#ifdef DEBUG
...
//      reps);
  hls::stream<ap_uint<26 * CONV_0_PE_DSP6 * 2> > conv_0_out(
      "conv_0_out");
#pragma HLS STREAM variable = conv_0_out depth = 128 dim = 1
  conv3x3_l0_bn_act_DSPopt_new<CONV_0_IFM_ROW, CONV_0_IFM_COL, CONV_0_IFM_CH,
                           CONV_0_IN_BIT, CONV_0_OFM_CH, CONV_0_OUT_BIT,
                           CONV_0_W_BIT, 26, CONV_0_INC_BIT_NEW,
                           CONV_0_BIAS_BIT_NEW, CONV_0_SIMD_DSP6, 3,
                           CONV_0_INPE, CONV_0_PE_DSP6, CONV_0_L_SHIFT>(
...
//      "pool_0_out");
//#pragma HLS STREAM variable = pool_0_out depth = 128 dim = 1
//  max_pool_gen0<CONV_0_OFM_ROW, CONV_0_OFM_COL, CONV_0_OFM_CH, 26, CONV_0_OUT_BIT,
//              CONV_0_PE_DSP6, CONV_0_INC_BIT_NEW, CONV_0_BIAS_BIT_NEW, CONV_0_W_BIT, CONV_0_L_SHIFT, CONV_0_IN_BIT>
//  	  	  	  (conv_0_out, pool_0_out, conv_0_inc_new, conv_0_bias_new, reps);

  hls::stream<ap_uint<CONV_0_OUT_BIT * CONV_0_PE_DSP6 * 2> > pool_0_out(
      "pool_0_out");
#pragma HLS STREAM variable = pool_0_out depth = 128 dim = 1
  max_pool2x2_onestep<CONV_0_OFM_ROW, CONV_0_OFM_COL, CONV_0_OFM_CH, 26, CONV_0_OUT_BIT,
              CONV_0_PE_DSP6, CONV_0_INC_BIT_NEW, CONV_0_BIAS_BIT_NEW, CONV_0_W_BIT, CONV_0_L_SHIFT, CONV_0_IN_BIT>
  	  	  	  (conv_0_out, pool_0_out, conv_0_inc_new, conv_0_bias_new, reps);
...
int main(int argc, char const *argv[]) {
  ...
  load_data("D:/LenovoSoftstore/Install/Xilinx/vivado_hls_project/pro0_scy_rgb/data/hebing16.bin", (char *)img, sizeof(img));
...
for(unsigned int r = 0; r < 4; r++)
{
  ...
  ultra_net(input_stream, output_stream);
